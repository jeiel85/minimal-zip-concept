...
fn test_corrupt_payload_checksum_mismatch() {
    ...
    let original_data =
        b"AAAAHelloBBBBWorldCCCCRepeatedDataZZZZ! This is highly secret binary codes!";
    let compressed_data = mzc::compress_bytes_v2(
        original_data,
    ...
    assert!(
        result.is_err(),
        "오염된 페이로드는 무결성 검증을 절대 통과해선 안 됩니다."
    );
}
...
fn test_mzc2_invalid_token_index() {
    ...
    assert!(
        result.is_err(),
        "이상 주소 토큰 참조는 디코더 선에서 즉시 차단되어야 합니다."
    );
    match result.unwrap_err() {
