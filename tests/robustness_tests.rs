...
fn test_decompress_bytes_v2_robustness_on_truncation() {
    ...
    for &(mode, entropy) in &configs {
        let compressed =
            mzc::compress_bytes_v2(original, mode, entropy, 6, true, true, false, false);
        ...
        for limit in 0..compressed.len() {
            ...
            assert!(
                result.is_ok(),
                "Panic detected on truncated MZC input with mode={:?}, entropy={:?} at limit {}",
                mode,
                entropy,
                limit
            );
...
fn test_decompress_bytes_v2_robustness_on_mutations() {
    ...
    for idx in 0..compressed.len() {
        ...
        for _ in 0..3 {
            ...
            assert!(
                result.is_ok(),
                "Panic detected on mutated byte at index {} in CM compressed binary",
                idx
            );
        }
    }
...
fn test_decompress_bytes_v2_robustness_on_completely_random_data() {
    ...
    for test_len in [0, 1, 5, 50, 56, 100, 1000, 10000] {
        for _ in 0..20 {
            ...
            assert!(
                result.is_ok(),
                "Panic detected on completely random MZC data of size {}",
                test_len
            );
        }
    }
...
fn test_deflate_decompress_robustness() {
    ...
    for test_len in [0, 1, 10, 18, 50, 500] {
        for _ in 0..50 {
            ...
            assert!(
                result_raw.is_ok(),
                "Panic detected on raw inflate with random size {}",
                test_len
            );
            ...
            assert!(
                result_gzip.is_ok(),
                "Panic detected on gzip_decompress with random size {}",
                test_len
            );
        }
    }
...
fn test_tans_decompress_robustness() {
    ...
    for test_len in [0, 1, 5, 20, 100, 1000] {
        for _ in 0..50 {
            ...
            assert!(
                result.is_ok(),
                "Panic detected on ans_decompress with random size {}",
                test_len
            );
        }
    }
...
fn test_cm_decompress_robustness() {
    ...
    for test_len in [0, 1, 5, 20, 100, 500] {
        for _ in 0..30 {
            ...
            assert!(
                result.is_ok(),
                "Panic detected on cm_decompress with random size {}",
                test_len
            );
        }
    }
