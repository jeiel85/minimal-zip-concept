...
./target/release/mzc compress input_file.bin output_file.mzip -m lz77 -e dynamic -l 9 --delta --bcj

# MZC6 tANS 및 사전 파일을 사용한 압축 실행
./target/release/mzc compress input_file.bin output_file.mzip -m lz77 -e ans -l 6 --dict-file trained.dict

# MZC7 컨텍스트 믹싱 및 PNG 필터를 사용한 압축 실행
./target/release/mzc compress input_image.png output_file.mzip -m hybrid -e cm --png
```

### 4.2 CLI 압축 해제 (`decompress`)
```bash
# SHA-256 검증 및 자동 버전/필터 식별 디코딩 복원 (내장 전역 사전 자동 추출)
./target/release/mzc decompress output_file.mzip restored_file.bin

# 외부 사전을 지정하여 해제하는 경우 (옵션)
./target/release/mzc decompress output_file.mzip restored_file.bin --dict-file trained.dict
```
...
```bash
./target/release/mzc inspect output_file.mzip
```
